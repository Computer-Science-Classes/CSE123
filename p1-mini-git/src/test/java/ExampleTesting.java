...
public class ExampleTesting {
    ...
    public void testSynchronizeOne() throws InterruptedException {
        // Initialize commit messages
        commitAll(repo1, new String[]{"Sheep", "Orca"});
        commitAll(repo2, new String[]{"Fish", "Bear"});
        ...
        testHistory(repo1, 4, new String[]{"Sheep", "Orca", "Fish", "Bear"});
    }
    ...
    public void commitAll(Repository repo, String[] messages) throws InterruptedException {
        ...
        for (String message : messages) {
            ...
            assertEquals(size + 1, repo.getRepoSize(), String
                    .format("Size not correctly updated after commiting message [%s]", message));
    ...
    public void testHistory(Repository repo, int n, String[] allCommits) {
        ...
        assertTrue(n <= totalCommits,
                String.format("Provided n [%d] too big. Only [%d] commits", n, totalCommits));
        ...
        assertTrue(nCommits.length <= allCommits.length,
                String.format("Not enough expected commits to check against. "
                        + "Expected at least [%d]. Actual [%d]", n, allCommits.length));
        ...
        for (int i = 0; i < n; i++) {
            ...
            assertTrue(commit.contains(commitMessage), String.format(
                    "Commit [%s] doesn't contain expected message [%s]", commit, commitMessage));
            assertTrue(commit.contains("" + backwardsIndex), String.format(
                    "Commit [%s] doesn't contain expected id [%d]", commit, backwardsIndex));
        }
    }
}

...
public class SearchClient {
    ...
    // Some class constants you can play around with to give random ratings to the
    // uploaded books!
    ...
    /**
     * Builds an inverted index from a collection of {@link Media}.
     *
     * The inverted index maps each normalized token to the set of {@code Media}
     * objects from whence it came.
     *
     * @param docs
     *            docs the list of media to index.
     * @return a map from tokens to sets of media containing those tokens.
     */
    public static Map<String, Set<Media>> createIndex(List<Media> docs) {
        Map<String, Set<Media>> invertedIndex = new TreeMap<>();

        for (Media m : docs) {
            List<String> content = m.getContent();

            for (String c : content) {
                c = c.toLowerCase();
                Set<Media> media = invertedIndex.get(c);
                if (media == null) {
                    media = new HashSet<>();
                    invertedIndex.put(c, media);
                }
                media.add(m);
            }
        }
    ...
    /**
     * Searches an inverted index for media matching a query token.
     *
     * @param index
     *            the inverted index to search.
     * @param query
     *            the query token.
     * @return a sorted set of media matching the query, or an empty set if none
     *         match.
     */
    ...
    // index - an inverted index mapping terms to the Set of media containing those
    // terms.
    // Should be non-null
    ...
    // Parameters:
    // console - the Scanner to get user input from. Should be non-null.
    // media - list of all media options loaded into the search engine. Should be
    // non-null.
    @SuppressWarnings("checkstyle:MissingJavadocMethod")
    ...
    // Exceptions:
    // FileNotFoundException - if BOOK_DIRECTORY does not exist or is not a
    // directory
    // Returns:
    ...
    // Adds ratings to the provided media numRatings amount of times. Each rating is
    // a random int
